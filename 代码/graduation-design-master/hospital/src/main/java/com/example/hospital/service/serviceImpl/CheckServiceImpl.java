...
public class CheckServiceImpl implements CheckService {
     ...
     */
    @Override
    public HashMap<String, Object> findAllChecks(int pageNumber, int size, String query,String chid) {
        ...
        if("1".equals(chid)){
            ...
            wrapper.like("ch_name", query).eq("ch_id",chid);
        }
    ...
    public List<Checks> selectsssChecks(Integer tid) {
        List<Checks>  sc = checkMapper.selectChecksGetKname();
        for (Checks c:sc) {
            List<Checks>  cs =  checkMapper.selectmyChecks(c.getKname(),tid);
    ...
    public Boolean addCheck(Checks checks){
        ...
        Subject subject = subjectMapper.selectByKname(checks.getKname());
        checks.setKid(subject.getKid());
        checks.setName(subject.getName());
    ...
    @Override
    public List<Checks> selectBykname(String kname) {
        return  checkMapper.selectBykname(kname);
    }
}
